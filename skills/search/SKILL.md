...
`$JOB_SEARCH_SHORTLIST_TARGET` is the maximum number of `shortlist` entries to have in the tracker at one time. **Check this before doing anything else.**

1. Count the current number of `shortlist` rows in `$JOB_SEARCH_TRACKER`.
2. If that count is **already ≥ target**, stop immediately — do not search, do not screen `found` entries. Report the count and exit.
3. After each new shortlist decision, recount. Stop as soon as the count reaches the target, even mid-session.

Counting rule: include ALL rows with status `shortlist`, regardless of when they were added. The target applies to the running total, not just jobs found this session.

Default target is 30 if the env var is unset.
...
| Rejected Co | Junior Analyst | NYC | junior/analyst | 2026-01-01 | $130k | screen_reject | Too junior. URL: https://... | | |
```
...
- `tailoring` — tailor agent is actively working on it
- `resume_ready` — tailored .docx files ready, queued for submission
- `hold` — paused, revisit later
- `applied` — successfully submitted
- `blocked` — transient failure during submission (CAPTCHA, login required, site down) — retry when resolved
...
Print one line per job as you process it — do not batch at the end:
...
✓ Citadel — Quant Risk VP  (shortlisted; credit/VaR/Python match; $350k; posted 1d ago)
```

Rules:
- `✓` for shortlisted, `✗` for screen_reject
- One line per job, no multi-line blocks
- Include the short reason in parentheses
- Print as you go — not all at the end
